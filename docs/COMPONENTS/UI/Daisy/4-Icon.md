...
## ✨ Contexto del componente Daisy Icon

## 1. 📌 Información General

`Daisy Icon` reutiliza el componente base:

`W4\UiFramework\Components\UI\Icon\Icon`

y aplica estilos/atributos a través del resolver:

`W4\UiFramework\Themes\DaisyUI\Components\UI\IconThemeResolver`
...
## 4. 🖥️ Formas de renderizar Daisy Icon

Nota de uso de tema:

- Usa `theme="daisyui"` en `x-w4-icon` cuando el tema global no sea DaisyUI.
- Usa `->theme('daisyui')` por helper/facade para forzar DaisyUI en esa instancia.
...
```blade
<x-w4-icon label="Visible" icon="heroicon-o-eye" theme="daisyui" />
<x-w4-icon label="Oculto" icon="heroicon-o-eye-slash" theme="daisyui" :hidden="true" />
```
